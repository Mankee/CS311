...
int main(int argc, char *argv[])
{
    ...
    if((opt = getopt(argc, argv, "vtqxd")) != -1) {
        switch (opt) {
            case 't':
...
void delete_files(int argc, char *argv[])
{
    ...
    if((old_arch_fd = open(argv[2], O_RDONLY)) == -1){
		...
		if((new_arch_fd = open("tmp_archive", O_CREAT | O_WRONLY | O_EXCL | O_APPEND, old_arch_stat.st_mode)) == -1){
            if(EEXIST){
            ...
            while((read(old_arch_fd, &header, 60)) != 0){
                ...
                if(file_position(argc, argv, &old_arch_fd, &header) == 0){
                    ...
                    if(atoi(header.ar_size) < BLOCKSIZE){
                        bytes_read = read(old_arch_fd, read_buff, atoi(header.ar_size));
                        if((write(new_arch_fd, read_buff, atoi(header.ar_size)) == -1)){
                            printf("Error(%d): %s\n", errno, strerror(errno));
                            exit(errno);
                        }
                        else{
                            printf("Writting file '%s'(%d bytes) to %s \n", header.ar_name, bytes_read, argv[2]);
                        }
                    }
                    else{
                        while(total_bytes_read <= atoi(header.ar_size) ){
                            ...
                            total_bytes_read += bytes_read;
                            if((write(new_arch_fd, read_buff, bytes_read) == -1)){
                                printf("Error(%d): %s\n", errno, strerror(errno));
                                exit(errno);
                            }
                            else{
